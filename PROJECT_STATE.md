...
- **Admin**: `app/admin/page.tsx` (server component, `force-dynamic`) lists responses + summary counts (Responses / Accepting / Declining / Guests coming), `noindex`. **Download for Excel** button → `app/admin/export/route.ts` (UTF-8 CSV with BOM so it opens cleanly in Excel; under `/admin/` so the same Basic Auth applies).
